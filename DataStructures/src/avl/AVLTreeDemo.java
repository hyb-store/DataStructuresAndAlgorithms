...
//        int[] arr = {4, 3, 6, 5, 7, 8};
        int[] arr = { 10, 11, 7, 6, 8, 9 }; // 单旋不能形成平衡二叉树，需要双旋
        //创建一个AVLTree
...
class AVLTree {
    ...
    public void delNode(int value) {
        if(root == null) {
            ...
            //如果要删除的结点是叶子结点     不能直接target = null，这只是删除了引用，对原数据没有任何影响
...
class Node {
    ...
    public void add(Node node) {
        ...
        if (node.value < this.value) {//添加的结点的值 小于 当前结点的值
            //如果当前结点左子结点为null
            if (this.left == null) {
                this.left = node;
            } else {//递归的向左子树添加
